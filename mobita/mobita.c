...
void CommandInventory(Mobita *m)
{
    ...
    printf("Enter Command: ");
    int cmd;
    scanf("%d", &cmd); //TEMPORARY SCANF
    //Guards Empty Bag
    if (cmd >= 1 && cmd <= 5 && GADGETOWNED(INVENTORY(*m), cmd - 1) == 0)
        cmd = 6;
    switch (cmd)
    {
    case 0:
    ...
    case 1:
        ...
        break;
    case 2:
        if (UseSenterPembesar(m))
            UpdateItem(&INVENTORY(*m), cmd - 1, -1);
        break;
    case 3:
        if (UsePintuKemanaSaja(m))
            UpdateItem(&INVENTORY(*m), cmd - 1, -1);
        break;
    case 4:
        if (UseMesinWaktu(m))
...
{
    printf("Command List\n");
    printf("1. Move -> Bergerak, bergerak, ayo selesaikan tugas! Command ini akan menampilkan lokasi yang bisa dicapai. \n");
    printf("2. Pick Up -> Ambil barang pesananmu, tetapi jangan salah ambil ya! \n");
    printf("3. Drop Off -> Antar barang pesananmu, tetapi jangan salah antar ya! \n");
    printf("4. To Do List -> Daftar yang harus kamu kerjakan, makanya ayo cepat !! >:( \n");
    ...
    printf("11. Help -> Uh.. \n");
    printf("12. Save -> Cape main? Sini-sini.\n");
    printf("13. Exit\n");
}
...
void CommandSave(Mobita *m)
{
    ...
    fprintf(fp, "%d\n", n);
    if (n > 0)
        for (i = daftarPesanan.idxHead; i <= daftarPesanan.idxTail; i++)
        {
...
{
    if (!isEmpty(TAS(*m)) && TOP(TAS(*m)).jenisItem == PERISHABLE)
        TOP(TAS(*m)).timeout = TOP(TAS(*m)).timeoutInitial;
...
{
    if (!isEmpty(TAS(*m)) && TOP(TAS(*m)).jenisItem == HEAVY)
        TOP(TAS(*m)).jenisItem = NORMAL;
